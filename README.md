...
**Control Voltage:** 24 VDC | **Power Supply:** 400 VAC, 3-Phase, 50 Hz  
...
9. [Cabling & Wiring Documentation](#9-cabling--wiring-documentation)
10. [I/O Channel Map](#10-io-channel-map)
11. [Safety & Emergency Stop](#11-safety--emergency-stop)
12. [Simulation Mode](#12-simulation-mode)
13. [Visualization & HMI](#13-visualization--hmi)
...
- **Deadlock-free** multi-station sequencing via reverse-priority scanning (station 7 → 1)
- **Emergency stop** that freezes all motion without losing state (resume-capable)
- **Simulation mode** for offline commissioning and lab testing without real I/O
- **Scalable documentation** — ePlan schematics, CODESYS project, and Excel cabling/wiring package all use the same project reference (PRJ-26001)
...
|----------|-------|---------------------------------|-----------|
| Start    | 0     | Load/input station (operator)   | —         |
...
| End      | 8     | Unload / completion position    | —         |
...
| On-board DI       | 8 × 24 VDC (X12, sinking, 2.8 mA, RC filter 5 µs)  |
| On-board DIO      | 8 × configurable DI/DO (X5)                         |
| On-board AI/AIO   | 2 × AI + 2 × AIO (X6)                              |
| On-board NI/PT    | 2 × NI1000/PT1000 (X13)                            |
...
| W102      | WAGO 852-1812   | X1        | EXT_NETWORK_UPLINK  | UTP  | 3 m    |
| W103      | WAGO 852-1812   | X8        | EXT_SERVICE_CON     | UTP  | 2 m    |
...
├── GVL                  (Global Variable List — shared I/O & flags)
...
| `GVL`        | Variable declaration | Shared I/O mapping across all POU's   |
...
| `xStationOccupied` | `ARRAY[1..7] OF BOOL` | Station occupancy flags (1=Bath1 … 7=Dryer)       |
...
| 2       | Set `xHoistBusy` flag (TRUE when state ≠ 0)                 |
...
| State | Name                   | Action                                                              |
|-------|------------------------|---------------------------------------------------------------------|
| `0`   | IDLE / Scan            | Reverse-priority scan (station 7→1→start) for the next job         |
...
| `45`  | Lower & Release        | Assert `xLowerHoist`; when lowered, open gripper, set occupancy    |
| `50`  | Lift Empty / Return    | De-assert `xLowerHoist`; returns to IDLE when fully lifted         |

### Priority Scheme (State 0 — IDLE)

Priorities are evaluated highest to lowest on every scan:

1. **Unload Dryer** — station 7 done → move to pos 8 (End)
2. **Advance station 6** — if station 7 is free
3. **Advance station 5** — if station 6 is free
4. **Advance station 4** — if station 5 is free
5. **Advance station 3** — if station 4 is free
6. **Advance station 2** — if station 3 is free
7. **Advance station 1** — if station 2 is free
8. **Pick new batch** — if `xMasterSwitch` AND `xStartSensor` AND station 1 free
...
```
IDLE → [State 10] Move to pickup pos → [State 20] Lower →
[State 25] Grip → [State 30] Lift → [State 40] Move to drop pos →
[State 45] Lower & Release → [State 50] Lift empty → IDLE
```
...
| Load picked from Start (pos 0) | No occupancy array used for pos 0            |

---

## 9. Cabling & Wiring Documentation
...
|-----------------------|---------------|-------------|---------------|
| 24V DC Positive       | Red           | 24V DC      | 20 AWG / 0.5 mm² |
| 0V DC Negative        | Black         | 0V DC       | 20 AWG / 0.5 mm² |
| Protective Earth (PE) | Green/Yellow  | Earth       | 20 AWG / 0.5 mm² |
| AC Line (L)           | Brown         | 230V AC     | 3G 1 mm²     |
| AC Neutral (N)        | Blue          | 0V AC       | 3G 1 mm²     |
...
- Minimum cable bending radius: **6× OD** (multi-core), **8× OD** (armoured)
...
|-----------|---------------|------------------|----------------|------------------------------|
| X11       | DI.1 – DI.8   | Digital Input    | `%IX0.0–IX0.7` | 24 VDC sinking, 2.8 mA, RC filter 5 µs |
| X5        | DIO.1 – DIO.8 | Configurable DIO | `%IX6.x / QX`  | Configurable as DI or DO     |
| X6        | AI.1–AI.2 + AIO.1–AIO.2 | Analog | `%IW` / `%QW` | 4–20 mA or 0–10 V        |
| X13       | NI/PT.1–NI/PT.2 | Temperature   | `%IW`          | NI1000 / PT1000              |

### Current I/O Assignments (from IO List sheet)

| Tag       | Connector | CODESYS Addr | Signal Type | Description              |
|-----------|-----------|--------------|-------------|--------------------------|
| X12:2     | X12       | —            | DC 0V       | PLC 0V reference         |
| X12:3     | X12       | `%IX6.0`     | DI 24VDC    | Control button 24V input |
| 24VIN-1   | Bus       | —            | 24V DC      | 24V DC Bus Input 1       |
| 24VIN-2   | Bus       | —            | 24V DC      | 24V DC Bus Input 2       |
| 24VIN-3   | Bus       | —            | 24V DC      | 24V DC Bus Input 3       |
...
- `GVL.xLowerHoist` is forced to `FALSE` (hoist stops descending)
- `GVL.xGrabLoad` is forced to `FALSE` (gripper deactivated)
- `GVL.iTargetPosition` is frozen to `GVL.iActualPosition` (no horizontal motion)
- `iSystemState` is **not reset** — the state machine resumes from where it was when the E-stop is cleared
...
- [ ] Validate HMI connectivity (Modbus TCP / WebVisu) over WAGO switch
...
*Last updated: 2026 | Rev A — Issued for Construction*